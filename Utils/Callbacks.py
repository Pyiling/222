...
class EarlyStopping(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        # 如果是最大化的指标，那么就计算改变量是否大于零，具体指标有acc，auc等
        max_improved = self.is_maximize and loss_change > self.tol
        # 如果是最小化的指标，则相反，具体有loss等
    ...
    def on_train_end(self, logs=None):
        ...
        self.trainer.best_loss = self.best_loss
...
class LRSchedulerCallback(Callback):
    ...
    任务调度回调函数，一般是在epoch层面，在每个epoch结束时调用
    ...
    def __post_init__(
        ...
        # 判断调整策略是否根据某一指标进行调整，例如ReduceLROnPlateau
