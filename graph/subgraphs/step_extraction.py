"""Subgraph for extracting investigation steps from document context."""
...
class StepExtractionState(TypedDict, total=False):
    document_id: str
...
#根据文档上下文提取排查步骤。
async def extract_steps(state: StepExtractionState, config: RunnableConfig) -> dict:
    settings = state["config"]
    outline = state.get("outline") or {}
    tools = state.get("tools") or []
    content = await _invoke_prompt(
        template_name="steps_extract",
        payload_data={
    ...
    if not plan or not isinstance(plan.get("steps"), list):
        raise ValueError("LLM 未能生成有效的排查步骤 JSON")
    logger.info(
        "extract_steps:done document_id=%s steps=%s",
        state.get("document_id"),
        len(plan.get("steps", [])),
    )
    return {"step_plan": plan}


workflow = StateGraph(StepExtractionState)
workflow.add_node("extract_steps", extract_steps)
workflow.add_edge(START, "extract_steps")
workflow.add_edge("extract_steps", END)

step_extraction_graph = workflow.compile()
