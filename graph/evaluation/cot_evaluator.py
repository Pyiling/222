...
async def aggregate_evaluation_results(state: EvaluationState, config: RunnableConfig) -> dict:
    ...
    # 计算加权总分（更新权重分配）
    weights = {
        "executability": 0.12,   # 工具可执行性
        "workflow": 0.12,        # 工作流合理性
        "necessity": 0.18,       # 工具必要性
        "causality": 0.20,       # 因果贡献
        "causal_chain": 0.18,    # 新增：工具链因果连贯性
        "correctness": 0.20      # 推理正确性
    }
    ...
    overall_evaluation = {
        ...
        "issues_found": all_issues[:20],  # 限制问题数量
        "strengths": correctness_results.get("strengths", []),
        "recommendations": correctness_results.get("suggestions", []),
        "causal_chain_analysis": {
            "total_pairs": len(state.get("tool_causal_chain_results", [])),
            "strong_causal_pairs": sum(1 for r in state.get("tool_causal_chain_results", [])
                                      if r.get("direct_causal", False) or r.get("causal_score", 0) >= 0.7),
            "weak_causal_pairs": sum(1 for r in state.get("tool_causal_chain_results", [])
                                    if r.get("causal_score", 0) < 0.6)
        },
        "summary": f"整体评估: {quality_grade} (分数: {overall_score:.2f})",
        "metadata": {
            "trajectory_length": len(state.get("trajectory", [])),
            "tool_steps": len([step for step in state.get("trajectory", []) if step.get("role") == "tool"]),
            "tools_used": len(set(step.get("tool_name") for step in state.get("trajectory", [])
                                  if step.get("role") == "tool")),
            "evaluation_duration": time.perf_counter() - start
        }
    }
...
workflow.add_edge("evaluate_tool_causal_chain", "evaluate_correctness")    # 修改
