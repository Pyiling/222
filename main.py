...
from graph.pipeline import pdf_pipeline_graph
from graph.pipeline_ocr import pdf_pipeline_ocr_graph
...
INPUT_FILE = Path(r"C:\Users\pan\Desktop\Data_Analyze_LiAgent-main\data_pipeline\data\datasets\sample_0.jsonl")
OUTPUT_FILE = INPUT_FILE.with_name(INPUT_FILE.stem + "_eval.json")
...
async def run_pipeline(pdf_path: str, document_id: str, parse_mode: str) -> None:
    settings = PipelineSettings()
    settings.validate()
    graph = pdf_pipeline_graph if parse_mode == "pdf" else pdf_pipeline_ocr_graph
    await graph.ainvoke(
        {
            "pdf_path": pdf_path,
...
def main() -> None:
    ...
    asyncio.run(run_pipeline(args.pdf_path, document_id, args.parse_mode))
